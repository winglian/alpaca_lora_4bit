...
def parse_commandline():
    ...
    # Config args group
    parser_config.add_argument("--config_file", type=str, default=None, required=False,
        help="YAML file to load for the configuration"
    )
...
def get_config() -> Finetune4bConfig:
    ...
    if args["config_file"]:
        ...
        return Finetune4bConfig(**yaml_data)

    config = Finetune4bConfig(
    ...
    # write out the current configuration to a YAML file, so it can be re-used
    with open("config.last.yaml", "w") as file:
